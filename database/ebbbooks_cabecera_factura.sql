...
CREATE TABLE `cabecera_factura` (
  ...
  `fk_detalleFactura` int(11) NOT NULL,
  `fecha` date NOT NULL,
  `total_pagar` double NOT NULL,
  PRIMARY KEY (`numero`),
  KEY `fk_cf_usuario_idx` (`fk_usuario`),
  KEY `fk_cf_df_idx` (`fk_detalleFactura`),
  CONSTRAINT `fk_cf_usuario` FOREIGN KEY (`fk_usuario`) REFERENCES `usuario` (`codigo`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_cf_df` FOREIGN KEY (`fk_detalleFactura`) REFERENCES `detalle_factura` (`numero`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
LOCK TABLES `cabecera_factura` WRITE;
